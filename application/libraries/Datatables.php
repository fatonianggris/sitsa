...
class Datatables {
    ...
    /**
     * Generates the initial value portion of the query
     *
     * @param string $table
     * @return mixed
     */
    ...
    private function get_filtering() {
        ...
        if (isset($mColArray)) {
            for ($i = 0, $ien = count($mColArray); $i < $ien; $i++) {
                $requestColumn = $mColArray[$i];
                $str = $requestColumn['search']['value'];

                if ($requestColumn['searchable'] == 'true' && $str != '') {
                    if ($this->check_cType())
                        $cWhere .= $this->select[$mColArray[$i]['name']] . " LIKE '%" . $str . "%' AND ";
                    else
                        $cWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $str . "%' AND ";
                }else {
                    $cWhere .= $this->select[$this->columns[$i]] . " LIKE '%" . $str . "%' AND ";
                }
            }
        ...
        $cWhere = substr_replace($cWhere, '', -4);

        if ($sWhere != '')
            $this->ci->db->where('(' . $sWhere . ')');

        if ($cWhere != '')
            $this->ci->db->where('(' . $cWhere . ')');
    ...
    private function get_total_results($filtering = FALSE) {
        ...
        $subquery = $this->ci->db->select($this->initial() . "*")->get_compiled_select($this->table);
        $countingsql = "SELECT COUNT(*) AS total FROM (" . $subquery . ") SqueryAux";
